...
**Konva 노드에서 크기를 읽지 마세요.** Transformer는 배율만 써 넣습니다
(`decompose()` 결과). 요소를 담는 `Group`은 width 속성이 없어 `node.width()`가
항상 0이라, 여기에 배율을 곱하면 조용히 0이 나옵니다. 크기는 요소의 현재 mm 값에
배율을 곱해 구하세요 — `resizedElement()`.

**창이 숨겨져 있으면 `requestAnimationFrame`이 멈춥니다.** 스크린샷도, Konva의
히트 캔버스도 안 그려져서 **합성 포인터 이벤트로 선택이 되지 않습니다.** 드래그는
되는데 클릭 선택만 안 되면 코드 문제가 아닙니다. 이럴 땐 라이브러리가 하는 일을
직접 재현하세요(노드에 `scaleX()` 쓰고 `_fire('transformend')`).
...
- **KC 마크나 KS 세탁 심볼을 코드로 그리지 마세요.** 등록된 지정 도형이라 근사해서
  만든 것을 제품에 붙이면 규제 위반입니다. 세탁 심볼은 여전히 자리표시 도형이고,
  사용자가 `image` 요소로 정품 파일을 올리게 하세요.
  **KC 마크는 예외입니다** — 2026-08-08에 소유자가 배포용 정품 파일을 제공하며
  기본 포함을 지시해, `public/kc-mark.png`를 KC 템플릿 3종이 씁니다. 이건
  "그리지 말라"의 반대가 아니라 그대로입니다: 여전히 그리지 않고, 정품 파일을
  씁니다. 비율은 절대 바꾸지 마세요 — `kcMark()`가 높이만 받는 이유입니다.
