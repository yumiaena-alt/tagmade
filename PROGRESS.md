...
최종 갱신: 2026-08-07 · 커밋 `8979ae1` · 브랜치 `feat/studio-workspace-layout`

> 이 브랜치는 아직 `main`에 머지되지 않았습니다. 라이브 사이트는 `main`(`2187076`)
> 기준이라 아래 레이아웃 변경이 반영돼 있지 않습니다.
...
상단 바(`PagePropertiesBar`)의 PDF 버튼 옆에 내보내기·가져오기를 붙이거나
`DocumentIo.tsx`를 새로 만드세요. 가져올 때 검증이 필요합니다 — `useDocumentStore`의 `merge`에 있는
...
| (이 커밋) | 홈을 에디터 한 페이지로 축소 — 마케팅 섹션 제거, H1을 캔버스 위로, 패널·캔버스 높이 정렬 |
